    ...
    <div className="relative min-h-screen">
      ...
      <div className="relative z-10 mx-auto grid min-h-screen max-w-6xl items-center gap-12 px-5 py-16 sm:px-8 lg:grid-cols-2">
        <div className="hidden lg:block">
          ...
          <p className="font-mono text-xs font-bold tracking-[0.45em] text-cyan-400">MATEMATIKA</p>
          <div className="relative my-2.5 w-full max-w-[340px] sm:max-w-[380px] -ml-1 group">
            {/* Ambient Radial Spotlight Glow for SIGMA */}
            <div
              className="pointer-events-none absolute -inset-4 rounded-full blur-2xl opacity-60"
              style={{
                background:
                  "radial-gradient(circle at 45% 50%, rgba(0, 240, 255, 0.35) 0%, rgba(36, 20, 102, 0.2) 60%, transparent 80%)",
              }}
            />
            <JusticeLeagueTitle className="scale-100 sm:scale-105 origin-left drop-shadow-[0_10px_25px_rgba(0,240,255,0.3)]" />
          </div>
          <p className="mt-3 font-display text-lg sm:text-xl font-bold tracking-widest text-white uppercase drop-shadow-sm">
            KELAS 11
          </p>
          <p className="mt-0.5 font-display text-sm sm:text-base font-semibold text-slate-300 tracking-wider">
            MAS DARUNNAJAH 9
          </p>
          ...
          <form onSubmit={submit} className="mt-6 space-y-4" data-testid="auth-form">
            ...
            {mode === "daftar" && role === "student" && (
              <div>
                ...
                <p className="mt-1 text-[11px] text-slate-400">Pilih salah satu dari 2 rombel kelas 11 yang terdaftar.</p>
              </div>
            )}
            <div>
